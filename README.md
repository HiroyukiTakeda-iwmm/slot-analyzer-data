...
> 🔴 **`source` 100% は「出典が確認できる」という意味ではない**（2026-08-17 実測）。
> 149台すべてが `source` を持つが、**URL 形式は0台**で、中身は「一撃、なな徹」のような
> **サイト名のみ**。どのページを見たのか・いつ取得したのかは記録されていない。
> つまり第三者が数値を追試できる状態にない。
>
> スキーマ上も `source` は **required ではない**（required は name / type / roles / author /
> version / lastUpdated の6つ）。したがって「出典なしのデータが入らない」保証は
> **どこにも存在しない**。恒久対策の案は `docs/data-provenance-proposal.md` を参照。
